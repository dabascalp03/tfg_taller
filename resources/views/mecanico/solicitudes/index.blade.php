            ...
            @foreach($solicitudes as $solicitud)
                <tr class="solicitud-row">
                    ...
                    <td>
                        ...
                        <form action="{{ route('mecanico.solicitudes.aprobar', $solicitud->id) }}" method="POST" style="display:inline-block;">
                        @csrf
                                @method('PATCH')
                                <button class="btn btn-success btn-sm">Aprobar</button>
                            </form>
                            <form action="{{ route('mecanico.solicitudes.rechazar', $solicitud->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('PATCH')

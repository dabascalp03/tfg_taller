        ...
        <tbody id="restorationTable">
            @foreach($restauraciones as $restauracion)
                <tr class="restoration-row">
                    ...
                    <td>
                        <a href="{{ route('admin.restauraciones.edit', $restauracion->id) }}" class="btn btn-primary btn-sm">Editar</a>

                        <form action="{{ route('admin.restauraciones.destroy', $restauracion->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
